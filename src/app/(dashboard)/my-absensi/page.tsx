...
export default function AbsensiPage() {
  ...
  const [incompleteAttendance, setIncompleteAttendance] =
    useState<TodayAttendance | null>(null);
  const [stats, setStats] = useState<AttendanceStats | null>(null);
  const [history, setHistory] = useState<AttendanceHistory[]>([]);
  const [approvedLeave, setApprovedLeave] = useState<ApprovedLeave | null>(
    null,
  );
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { data: historyData, error: historyError } = await supabase
        .from("absensi")
        .select(
          "tanggal, check_in_time, check_out_time, status, check_in_keterangan",
        )
        .eq("user_id", user.id)
        .gte(
          "tanggal",
      ...
      const { data: leaveData, error: leaveError } = await supabase.rpc(
        "check_approved_leave_for_date",
        {
          p_user_id: user.id,
          p_date: getTodayDate(),
        },
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span
        className={`px-3 py-1 text-xs font-bold rounded-full ${statusInfo.className}`}
      >
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-10">
      ...
      <div className="text-center px-6 mb-8">
        ...
        >
          <span
            className={`w-2 h-2 rounded-full ${hasCheckedOut ? "bg-gray-400" : hasCheckedIn ? "bg-emerald-500" : "bg-slate-400"}`}
          ></span>
          ...
          <div className="flex justify-between divide-x divide-gray-100">
            <div className="text-center px-2 flex-1">
              ...
              <p className="text-lg font-bold text-slate-800">
                {hasCheckedIn
                  ? formatTime(new Date(todayAttendance!.check_in_time!)).slice(
                      0,
                      5,
                    )
                  : "--:--"}
        ...
        <div className="px-6 mb-6">
          ...
          <div className="grid grid-cols-5 gap-2">
            <div className="text-center p-2 bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-100/50">
              ...
              <p className="text-[10px] font-medium text-green-600/80 mt-0.5">
                Hadir
              </p>
            </div>
            ...
            <div className="text-center p-2 bg-yellow-50 dark:bg-yellow-900/20 rounded-xl border border-yellow-100/50">
              ...
              <p className="text-[10px] font-medium text-yellow-600/80 mt-0.5">
                Sakit
              </p>
            </div>
            <div className="text-center p-2 bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-100/50">
              ...
              <p className="text-[10px] font-medium text-red-600/80 mt-0.5">
                Alpha
              </p>
            </div>
      ...
      {((hasCheckedIn && todayAttendance?.check_in_time) ||
        ...
        <CheckOutDialog
          ...
          checkInTime={
            incompleteAttendance
              ? incompleteAttendance.check_in_time ||
                new Date(incompleteAttendance.tanggal).toISOString()
