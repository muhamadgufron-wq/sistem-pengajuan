...
export default function AdminPage() {
  ...
  const [displayApprovedAmount, setDisplayApprovedAmount] =
    useState<string>("");
  ...
  const fetchData = useCallback(async () => {
    ...
    const { data: barangData, error: barangError } = await supabase.rpc(
      "v2_get_all_barang_submissions",
      rpcParams
    );
    const { data: uangData, error: uangError } = await supabase.rpc(
  ...
  // Upload transfer proof to storage
  const uploadTransferProof = async (pengajuanId: number, userId: string): Promise<string | null> => {
    if (!transferProofFile) return null;

    try {
      const fileExt = transferProofFile.name.split('.').pop();
      const filename = `${userId}/${pengajuanId}_${Date.now()}.${fileExt}`;
  ...
  const handleUpdate = async () => {
    ...
    if (transferProofFile && activeTab === 'uang' && newStatus === 'disetujui') {
      ...
      try {
        ...
        if (proofUrl) {
          updateData.bukti_transfer_url = proofUrl;
        }
      } catch (error) {
  ...
  const openUpdateDialog = (item: PengajuanItem) => {
    ...
    if ("jumlah_uang" in item) {
      ...
      const quantity =
        ...
        (item as PengajuanBarang).jumlah;
  ...
  const handleApprovedAmountChange = (
  ...
  const kategoriUangOptions = [
    { value: "operasional", label: "Operasional" },
    { value: "vendor", label: "Vendor" },
    { value: "kasbon", label: "Kasbon" },
  ];
  const kategoriFilterOptions =
    activeTab === "barang" ? kategoriBarangOptions : kategoriUangOptions;
  const kategoriDialogOptions =
    activeTab === "barang" ? kategoriBarangOptions : kategoriUangOptions;

  // Helper function to get public URL for transfer proof
  const getTransferProofUrl = (path: string | null) => {
    if (!path) return null;
  ...
  return (
    <>
        ...
        <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-xl">
              Update Pengajuan #{editingItem?.id}
            </DialogTitle>
            <DialogDescription>
              Diajukan oleh: {editingItem?.full_name || "-"} |{" "}
              {editingItem && formatDate(editingItem.created_at)}
            </DialogDescription>
          </DialogHeader>
          <div className="my-2 space-y-2 border bg-muted/30 p-3 rounded-md">
            ...
            {editingItem && "nama_barang" in editingItem && (
              <p className="text-sm text-muted-foreground">
                {editingItem.nama_barang} ({editingItem.jumlah} unit)
              </p>
            )}
            {editingItem && "jumlah_uang" in editingItem && (
              <p className="text-sm text-muted-foreground">
                Rp {Number(editingItem.jumlah_uang).toLocaleString("id-ID")} -{" "}
          ...
          <div className="grid gap-4 py-4 md:grid-cols-2">
            ...
            {editingItem && "jumlah_uang" in editingItem && (
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Diminta: Rp{" "}
                  {Number(
                    (editingItem as PengajuanUang).jumlah_uang
            ...
            {/* Upload Bukti Transfer - Only for Uang and Disetujui */}
        ...
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle className="text-2xl">
              Detail Pengajuan #{viewingItem?.id}
            </DialogTitle>
            <DialogDescription>
              Diajukan oleh:{" "}
              <span className="font-semibold">{viewingItem?.full_name}</span>{" "}
              pada {viewingItem && formatDate(viewingItem.created_at)}
            </DialogDescription>
          </DialogHeader>
                ...
                <>
                  ...
                  <div className="grid grid-cols-3 items-center gap-4 border-b pb-2">
                    ...
                    <span className="col-span-2">
                      ...
                      viewingItem.jumlah_disetujui !== viewingItem.jumlah ? (
                        <div>
                          <div className="font-medium text-green-600">
                            {viewingItem.jumlah_disetujui} unit (disetujui)
                          </div>
                          <div className="text-sm text-muted-foreground line-through">
                            {viewingItem.jumlah} unit (diminta)
                          </div>
                        </div>
                      ) : (
                  ...
                  <div className="grid grid-cols-3 items-start gap-4 border-b pb-2">
                    ...
                    <span className="col-span-2">{viewingItem.alasan}</span>
                  </div>
                </>
              )}
          ...
          <DialogFooter>
            <Button variant="destructive" onClick={() => setViewingItem(null)}>
              Tutup
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      <Dialog
        open={!!buktiItem}
        onOpenChange={(open) => !open && setBuktiItem(null)}
      >
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>Bukti Laporan</DialogTitle>
            <DialogDescription>Pengajuan #{buktiItem?.id}</DialogDescription>
          </DialogHeader>
          <div className="py-2">
            {buktiItem && <ProofGallery pengajuanId={buktiItem.id} />}
          </div>
        </DialogContent>
      </Dialog>
            ...
            <CardContent>
              <div className="flex flex-wrap items-center justify-between gap-y-4">
                <div className="flex flex-wrap item-center gap-4">
                  ...
                  <Select
                    onValueChange={(value) =>
                      setKategoriFilter(value === "all" ? "" : value)
                    }
                ...
                <div className="flex items-center gap-4">
                  ...
                  <Button onClick={handleFilterReset} variant="outline">
                    <RefreshCcwIcon className="mr-2 h-4 w-4" /> Reset
                  </Button>
                </div>
              </div>
            </CardContent>
